...
def _write_summary(summary: dict, per_file: list) -> None:
    lines = [
        ...
        "",
        f"  {'file':<48} {'market':<12} {'rows':>5} {'null_edges':>10} {'schema_errors':>15} {'status':>14}",
    ]

    for pf in per_file:
        lines.append(
            f"  {pf['name']:<48} {pf['market']:<12} {pf['rows']:>5} "
            f"{pf['null_edges']:>10} {pf['schema_errors']:>15} {pf['status']:>14}"
        )
...
def validate_forbidden_columns(df: pd.DataFrame, forbidden_columns: list, label: str) -> None:
    ...
    if present:
        raise ValueError(
            ...
            f"Run-line edge math must use home_normalized_prob_run_line / away_normalized_prob_run_line."
        )
...
def main():
    ...
    try:
        ...
        print(f"compute_edges failed. Could not load config: {CONFIG_PATH}")
        raise SystemExit(1)
    ...
    for input_file in input_files:
        ...
        try:
            ...
            _log(f"{input_file.name} SCHEMA FAILED: {e}\n{traceback.format_exc()}", "ERROR")
