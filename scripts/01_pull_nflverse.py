...
Self-heals if the installed nfl_data_py is too old by upgrading in-place.
...
def ensure_pkg():
    """Make sure nfl_data_py has import_rosters; upgrade package if missing."""
    try:
    ...
    if not hasattr(nfl, "import_rosters"):
        print("Upgrading nfl_data_py ...", flush=True)
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "nfl_data_py"])
        import nfl_data_py as nfl2
        importlib.reload(nfl2)
        globals()["nfl"] = nfl2
    else:
        globals()["nfl"] = nfl

    if not hasattr(globals()["nfl"], "import_rosters"):
        avail = [x for x in dir(globals()["nfl"]) if x.startswith("import_")]
        raise AttributeError(f"'nfl_data_py' missing import_rosters. Available: {avail}")
