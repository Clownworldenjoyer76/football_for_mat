#!/usr/bin/env python3
"""
NFL weekly historical spread-prediction enrichment.

The rule definitions, bucket boundaries, historical rates, and action
directions remain data-driven from spread_enrichment.csv.
"""
...
class SpreadEnrichmentError(RuntimeError):
    pass
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build NFL historical spread enrichment for "
...
def validate_master(
    ...
    require_columns(
        headers,
        MASTER_REQUIRED_HEADERS,
        label="spread enrichment master",
    )

    if not rows:
        fail(
            "Spread enrichment master contains no rows"
        )
    ...
    for line_number, rule in enumerate(
        ...
        if not rule_id:
            fail(
                f"Spread master line {line_number} "
                "has blank rule_id"
            )

        if rule_id in seen_ids:
            fail(
                f"Spread master contains duplicate "
                f"rule_id={rule_id}"
            )
        ...
        if active not in {"0", "1"}:
            fail(
                f"Spread master rule_id={rule_id} "
    ...
    if not active_supported_ids:
        fail(
            "Spread enrichment master has no active "
...
def build_summary_fields(
    ...
    for side_name, side in [
        ...
        for label, item in [
            ...
            g[
                f"{side_name}_{label}_hist_cover_rate_pct"
            ] = (
                item["historical_cover_rate_pct"]
                if item
                else ""
            )
            g[
...
def build_staged_root(
    *,
    ...
    stage_root = Path(
        tempfile.mkdtemp(
            prefix=".spread_enrichment_stage_",
    ...
    try:
        ...
        for week, (
            ...
            if (
                ...
                fail(
                    "Staged spread enrichment differs "
