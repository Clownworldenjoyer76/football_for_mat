...
DRAT_REL = Path(
    "docs/win/football/nfl/00_intake/predictions/drat"
)
ODDS_REL = Path(
...
def find_drat_file(
    ...
    matches = [
        path
        for path in drat_dir.glob(
            f"{season}_wk*_odds.csv"
        )
    ...
    if not matches:
        raise FileNotFoundError(
            f"No DRAT file matching season={season}, "
            f"week={week} found in {drat_dir}"
        )

    raise RuntimeError(
        f"Multiple DRAT files match season={season}, "
        f"week={week}: "
        + ", ".join(
            path.name
            for path in matches
