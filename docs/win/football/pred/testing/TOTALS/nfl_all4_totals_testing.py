...
- Final scores and market totals are used only to create the historical Over/Under target.
- Every eligible family/game is tested once as Over and once as Under.
- Exact-total pushes are excluded from totals hit-rate calculations.
- All outputs are written to the TOTALS testing folder.
"""
...
# Same actionable thresholds as the ATS historical test.
MIN_GAMES = 50
MIN_SEASONS = 3
MIN_ABS_LIFT_PP = 3.0
...
def build_features(drat, epred, odds, schedule):
    ...
    # Historical totals target inputs. Final Over/Under result is created later.
...
def build_selection_rows(f: pd.DataFrame) -> pd.DataFrame:
    ...
    def add_family(family, side, prob, mask=None):
        ...
        # Totals testing requires a market total and final scores.
        sub = sub[sub["odds_total"].notna() & sub["actual_total"].notna()].copy()
        if sub.empty:
            return

        # Keep the same family-selected-side context/features used by the ATS test.
        ...
        # Test every eligible family/game twice: once Over and once Under.
        for totals_direction in ["Over", "Under"]:
            d = sub.copy()
            d["TotalsDirection"] = totals_direction
            d["TotalsMargin"] = np.where(
                totals_direction == "Over",
                d["actual_total"] - d["odds_total"],
                d["odds_total"] - d["actual_total"],
            )
            d["TotalsResult"] = np.where(
                d["actual_total"] == d["odds_total"], "Push",
                np.where(d["TotalsMargin"] > 0, "Win", "Loss")
            )
            d["Hit"] = np.where(
                d["TotalsResult"].eq("Win"), 1,
                np.where(d["TotalsResult"].eq("Loss"), 0, np.nan)
            )

            keep = [
                "Family", "TotalsDirection", "game_id", "season", "week", "season_type",
                "SelectedSide", "SelectedTeam", "TotalsResult", "Hit", "TotalsMargin",
            ...
            rows.append(d[keep].rename(columns={
...
def test_condition(family_df, condition_df, family_baseline):
    family_df = family_df[family_df["Hit"].notna()].copy()
    condition_df = condition_df[condition_df["Hit"].notna()].copy()

    n = len(condition_df)
    wins = int(condition_df["Hit"].sum())
    ...
    fam_season = family_df.groupby("season")["Hit"].agg(["count", "sum"])
    fam_season["pct"] = 100 * fam_season["sum"] / fam_season["count"]

    grp_season = condition_df.groupby("season")["Hit"].agg(["count", "sum"])
    ...
    return {
        ...
        "Family_Baseline_HitPct": round(family_baseline, 2),
...
def run_tests(sel: pd.DataFrame):
    ...
    # Same deliberate interactions as the ATS historical test.
    interactions = [
    ...
    # Separate Over and Under baselines for every family.
    for (family, totals_direction), fam in sel.groupby(
        ["Family", "TotalsDirection"], sort=False
    ):
        fam = fam.copy()
        decided = fam[fam["Hit"].notna()].copy()
        if decided.empty:
            continue

        baseline = 100 * decided["Hit"].mean()
        baselines.append({
            "Family": family,
            "TotalsDirection": totals_direction,
            "Games": len(decided),
            "Wins": int(decided["Hit"].sum()),
            "Losses": int(len(decided) - decided["Hit"].sum()),
            "Pushes_Excluded": int(fam["Hit"].isna().sum()),
    ...
    actionable = actionable.assign(
        ...
        ["__abs_lift", "Games"], ascending=[False, False]
    ).drop(columns="__abs_lift")
...
def build_implementation(actionable: pd.DataFrame):
    if actionable.empty:
        return pd.DataFrame(columns=[
            "Family", "Totals_Direction", "Feature_Test", "Historical_Condition",
            "Direction", "Historical_Hit_Pct", "Lift_Points", "Games",
    ...
    return out[[
        "Family", "TotalsDirection", "Test", "Condition", "Direction", "Hit_Pct",
        "Lift_vs_Family", "Games", "Season_Consistency_Pct", "Forward_Record",
        "Pipeline_Action",
    ]].rename(columns={
        "TotalsDirection": "Totals_Direction",
        "Test": "Feature_Test",
...
def main():
    ...
    print("[3/5] Building DRAT / EPRED / market / consensus Over/Under rows...", flush=True)
    selections = build_selection_rows(features)
    if selections.empty:
        raise SystemExit("No totals selection rows remained after requiring market totals and final scores.")
    ...
    lines.append(f"Totals selection rows (Over + Under): {len(selections)}")
    ...
    if actionable.empty:
        ...
        for _, r in top.iterrows():
            lines.append(
                f"{r['Family']} {r['TotalsDirection']} | {r['Condition']} | "
                f"{int(r['Wins'])}-{int(r['Losses'])} | {r['Hit_Pct']}% | "
