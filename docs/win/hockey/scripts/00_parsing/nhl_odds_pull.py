...
JSON_OUT_DIR = Path("docs/win/hockey/odds/nhl")
SPORTSBOOK_OUT_DIR = Path("docs/win/hockey/00_intake/sportsbook")
SPORTSBOOK_NHL_OUT_DIR = Path("docs/win/hockey/00_intake/sportsbook/nhl")
LOG_DIR = Path("docs/win/hockey/errors/00_intake")

JSON_OUT_DIR.mkdir(parents=True, exist_ok=True)
SPORTSBOOK_OUT_DIR.mkdir(parents=True, exist_ok=True)
SPORTSBOOK_NHL_OUT_DIR.mkdir(parents=True, exist_ok=True)
LOG_DIR.mkdir(parents=True, exist_ok=True)

LOG_FILE = LOG_DIR / "odds_api_nhl_pull.txt"

with open(LOG_FILE, "w", encoding="utf-8") as f:
    f.write(f"=== odds_api_nhl_pull RUN {datetime.now(ET).isoformat()} ===\n")
...
def build_rows(event: dict, odds_payload: dict) -> tuple[dict, dict, dict]:
...
def main() -> None:
    ...
    raw_records = []
    normalized_rows = []
    csv1_by_date = defaultdict(list)
    csv2_by_date = defaultdict(list)
    ...
    for event in events:
        ...
        try:
            ...
            normalized, csv1, csv2 = build_rows(event, odds_payload)
            ...
            if game_date:
                csv1_by_date[game_date].append(csv1)
                csv2_by_date[game_date].append(csv2)
    ...
    for game_date, rows in sorted(csv1_by_date.items()):
        write_csv(
            SPORTSBOOK_OUT_DIR / f"hockey_{game_date}.csv",
            CSV1_FIELDS,
            rows,
        )

    for game_date, rows in sorted(csv2_by_date.items()):
        write_csv(
            SPORTSBOOK_NHL_OUT_DIR / f"nhl_{game_date}.csv",
            CSV2_FIELDS,
            rows,
        )
    ...
    log(f"CSV sportsbook dates: {len(csv1_by_date)}")
    log(f"CSV sportsbook/nhl dates: {len(csv2_by_date)}")

    if events and not normalized_rows:
        log("STATUS: FAILED")
        raise RuntimeError("Events were found but no odds rows were written")

    log("STATUS: SUCCESS")
    print(f"WROTE {json_out_path} rows={len(normalized_rows)}")
    print(f"WROTE {len(csv1_by_date)} sportsbook CSV date file(s)")
    print(f"WROTE {len(csv2_by_date)} sportsbook/nhl CSV date file(s)")
