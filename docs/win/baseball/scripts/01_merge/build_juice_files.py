...
FORBIDDEN_RUN_LINE_INPUT_COLUMNS = [
    "home_run_line_prob",
    "away_run_line_prob",
]
...
def validate_no_duplicate_columns(df, file_path, stage):
    duplicates = duplicate_columns(list(df.columns))

    if duplicates:
        log(f"SCHEMA ERROR: {file_path} duplicate columns during {stage}: {duplicates}")
        return False

    return True


def validate_required_columns(df, required_columns, file_path):
...
def validate_forbidden_columns_absent(df, forbidden_columns, file_path):
    present = [c for c in forbidden_columns if c in df.columns]

    if present:
        log(
            f"SCHEMA ERROR: {file_path} contains obsolete forbidden columns: {present}. "
            f"These fields must not be produced by merge_intake.py."
        )
        return False
...
def validate_input_schema(df, required_columns, file_path, forbidden_columns=None):
    ok = True

    if not validate_no_duplicate_columns(df, file_path, "input"):
        ok = False

    if not validate_required_columns(df, required_columns, file_path):
        ok = False

    if forbidden_columns and not validate_forbidden_columns_absent(df, forbidden_columns, file_path):
        ok = False

    return ok


def validate_output_schema(df, file_path):
    return validate_no_duplicate_columns(df, file_path, "output")


def write_csv_checked(df, out_path):
    if not validate_output_schema(df, out_path):
        raise RuntimeError(f"output schema validation failed for {out_path}")
...
def process_moneyline(file_path, summary):
    try:
        ...
        if not validate_input_schema(df, MONEYLINE_REQUIRED_COLUMNS, file_path):
...
def process_total(file_path, summary):
    try:
        ...
        if not validate_input_schema(df, TOTAL_REQUIRED_COLUMNS, file_path):
            summary["schema_errors"] += 1
            return
...
def process_run_line(file_path, summary):
    try:
        ...
        if not validate_input_schema(
            df,
            RUN_LINE_REQUIRED_COLUMNS,
            file_path,
            forbidden_columns=FORBIDDEN_RUN_LINE_INPUT_COLUMNS,
        ):
            summary["schema_errors"] += 1
            return
