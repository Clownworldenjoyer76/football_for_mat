...
def http_get_json(
    ...
    for attempt in range(
        ...
        request = Request(
            ...
            headers={
                "User-Agent":
                "nfl-pull-opening-odds-espn/1.0"
            },
        )
...
def earliest_snapshot_timestamp(
    event_id: str,
    bookmaker: str,
) -> str:
    earliest = ""
    ...
    for row in weekly_rows:
        ...
        try:
            ...
            log(
                ...
                f"{row.get('game_id', '')} "
                f"event_id="
                f"{row.get('odds_provider_game_id', '')} "
                f"error={exc}"
            )
...
def replace_current_week_rows(
    ...
    for row in new_rows:
        ...
        if previous_same_key:
            previous_timestamp = str(
                previous_same_key.get(
                    "opening_timestamp",
                    "",
                )
            ).strip()
...
def main() -> None:
    ...
    log(
        "Current ESPN games replacing "
        "legacy rows: "
        f"{len(replace_game_ids)}"
    )

    log(
        "Existing current-game rows "
        "removed before ESPN replacement: "
        f"{removed_count}"
    )

    log(
        f"New ESPN opener rows built: "
        f"{len(new_rows)}"
    )

    log(
        f"Current ESPN rows written: "
        f"{len(current_game_rows)}"
    )

    log(
        f"Current ESPN ok rows: "
        f"{current_game_ok}"
    )

    log(
        f"Current ESPN missing rows: "
        f"{current_game_missing}"
    )

    log(
        f"Current ESPN error rows: "
        f"{current_game_error}"
    )

    log(
        f"Final season opener rows written: "
        f"{len(final_rows)}"
    )

    log(
        f"Final season opener ok rows: "
        f"{ok_rows}"
    )

    log(
        f"Final season opener missing rows: "
        f"{missing_rows}"
    )

    log(
        f"Final season opener error rows: "
        f"{error_rows}"
    )
