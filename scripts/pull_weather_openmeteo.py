...
Usage:
  python scripts/pull_weather.py --season 2025 --week 1
  python scripts/pull_weather.py                   # next 10 days
"""
...
STADIUMS = {
    # AFC East
    "BUF": {"lat": 42.7738, "lon": -78.7875, "dome": False},  # Highmark
    "MIA": {"lat": 25.9580, "lon": -80.2389, "dome": False},  # Hard Rock
    "NE":  {"lat": 42.0909, "lon": -71.2643, "dome": False},  # Gillette
    "NYJ": {"lat": 40.8128, "lon": -74.0742, "dome": False},  # MetLife
    # AFC North
    ...
    "IND": {"lat": 39.7601, "lon": -86.1639, "dome": True},   # Lucas Oil
...
def read_schedules() -> pd.DataFrame:
    ...
    for p in paths:
        try:
            ...
            pass
    if not dfs:
        raise SystemExit("Could not read any schedules files.")
    df = pd.concat(dfs, ignore_index=True)
    df.columns = [c.strip().lower() for c in df.columns]

    # kickoff_utc assembly
...
def select_games(df: pd.DataFrame, season: int|None, week: int|None) -> pd.DataFrame:
    ...
    keep = [c for c in ["game_id","season","week","home_team","away_team","roof","kickoff_utc"] if c in sel.columns]
...
def main():
    ...
    if games.empty:
        ...
        print(f"Wrote empty: {OUT_LATEST}")
        return
    ...
    for _, g in games.iterrows():
        ...
        roof_val = str(g.get("roof", "")).strip().lower()
