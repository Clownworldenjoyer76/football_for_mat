...
def _write_summary(summary: dict, per_file: list) -> None:
    lines = [
        ...
        "",
        f"  {'file':<45} {'rows':>5} {'applied':>8} "
        f"{'skipped_bad':>12} {'skipped_noband':>15} {'schema_errors':>15}",
    ]
...
def process_row(df, juice_df, idx, row):
    ...
    if not math.isfinite(home_american) or not math.isfinite(away_american):
        _log(f"row={idx} reason=invalid_american_odds home={home_american} away={away_american}", "SKIP")
        return df, "bad"
...
def main():
    ...
    try:
        ...
        if not bad_juice_rows.empty:
            _log(f"juice file has {len(bad_juice_rows)} invalid rows after numeric/string coercion", "ERROR")
        ...
        for file_path in files:
            ...
            try:
                ...
                _log(
                    ...
                    f"schema_errors={pf['schema_errors']}"
                )
