#!/usr/bin/env python
"""
NFL all-4 ATS historical testing + pipeline feature builder.
...
- Final scores and spreads are used only to create the historical ATS target.
- Pushes are excluded from ATS rate calculations.
...
DEFAULT_OUT = r"C:\Users\Mat\Documents\GitHub\football_for_mat\docs\win\football\pred\testing_ats"
...
def build_features(drat, epred, odds, schedule):
    ...
    # Historical ATS target inputs. Final ATS result is created per selected side.
    f["home_margin"] = f["__home_score"] - f["__away_score"]
...
def build_selection_rows(f: pd.DataFrame) -> pd.DataFrame:
    ...
    def add_family(family, side, prob, mask=None):
        ...
        sub = sub[sub["SelectedSpread"].notna()].copy()
        if sub.empty:
            return

        sub["SelectedScore"] = selected_value(
            sub, "SelectedSide", "__home_score", "__away_score"
        )
        sub["OpponentScore"] = selected_value(
            sub, "SelectedSide", "__away_score", "__home_score"
        )
        sub["ATSMargin"] = sub["SelectedScore"] - sub["OpponentScore"] + sub["SelectedSpread"]
        sub["ATSResult"] = np.where(
            sub["ATSMargin"] > 0, "Cover",
            np.where(sub["ATSMargin"] < 0, "Loss", "Push")
        )
        sub["Covered"] = np.where(
            sub["ATSResult"].eq("Cover"), 1,
            np.where(sub["ATSResult"].eq("Loss"), 0, np.nan)
        )
        ...
        keep = [
            ...
            "SelectedSide", "SelectedTeam", "ATSResult", "Covered", "ATSMargin", "SelectedSpread", "SelectedProb",
...
def test_condition(family_df, condition_df, family_baseline):
    family_df = family_df[family_df["Covered"].notna()].copy()
    condition_df = condition_df[condition_df["Covered"].notna()].copy()

    n = len(condition_df)
    covers = int(condition_df["Covered"].sum())
    pct = 100 * covers / n if n else np.nan
    lift = pct - family_baseline if n else np.nan

    low_center, half = wilson_interval(covers, n)
    ci_low = low_center - half
    ci_high = low_center + half

    fam_season = family_df.groupby("season")["Covered"].agg(["count", "sum"])
    fam_season["pct"] = 100 * fam_season["sum"] / fam_season["count"]

    grp_season = condition_df.groupby("season")["Covered"].agg(["count", "sum"])
    ...
    return {
        ...
        "Covers": covers,
        "ATS_Losses": n - covers,
        "Cover_Pct": round(pct, 2) if pd.notna(pct) else np.nan,
        "Wilson95_Low": round(ci_low, 2) if pd.notna(ci_low) else np.nan,
        "Wilson95_High": round(ci_high, 2) if pd.notna(ci_high) else np.nan,
        "Family_Baseline_CoverPct": round(family_baseline, 2),
        "Lift_vs_Family": round(lift, 2) if pd.notna(lift) else np.nan,
...
def run_tests(sel: pd.DataFrame):
    ...
    for family, fam in sel.groupby("Family", sort=False):
        fam = fam.copy()
        decided = fam[fam["Covered"].notna()].copy()
        baseline = 100 * decided["Covered"].mean()
        baselines.append({
            "Family": family,
            "Games": len(decided),
            "Covers": int(decided["Covered"].sum()),
            "ATS_Losses": int(len(decided) - decided["Covered"].sum()),
            "Pushes_Excluded": int(fam["Covered"].isna().sum()),
            "Cover_Pct": round(baseline, 2),
        })
    ...
    baselines = pd.DataFrame(baselines).sort_values("Cover_Pct", ascending=False)
...
def build_implementation(actionable: pd.DataFrame):
    if actionable.empty:
        return pd.DataFrame(columns=[
            ...
            "Historical_Cover_Pct", "Lift_Points", "Games", "Forward_Record",
            "Pipeline_Action",
        ])
...
def main():
    ...
    features.to_csv(out_dir / "ats_historical_game_features.csv", index=False)
    issues.to_csv(out_dir / "join_issues.csv", index=False)

    print(f"      usable joined games: {len(features)}", flush=True)

    print("[3/5] Building DRAT / EPRED / market / consensus ATS selection rows...", flush=True)
    selections = build_selection_rows(features)
    selections.to_csv(out_dir / "ats_selection_rows.csv", index=False)
    ...
    baselines.to_csv(out_dir / "ats_source_baselines.csv", index=False)
    full_tests.to_csv(out_dir / "ats_full_test_results.csv", index=False)
    actionable.to_csv(out_dir / "ats_actionable_findings.csv", index=False)

    print("[5/5] Writing pipeline implementation output...", flush=True)
    implementation = build_implementation(actionable)
    implementation.to_csv(out_dir / "ats_pipeline_implementation.csv", index=False)

    lines = []
    lines.append("NFL FOUR-DATASET ATS HISTORICAL TESTING")
    lines.append("=" * 38)
    lines.append(f"Usable joined historical games: {len(features)}")
    lines.append(f"Actionable repeatable ATS findings: {len(actionable)}")
    lines.append("")
    lines.append("SOURCE / CONSENSUS ATS BASELINES")
    for _, r in baselines.iterrows():
        lines.append(
            f"{r['Family']}: {int(r['Covers'])}-{int(r['ATS_Losses'])} ATS = {r['Cover_Pct']}% | pushes excluded {int(r['Pushes_Excluded'])}"
        )
    ...
    if actionable.empty:
        ...
        for _, r in top.iterrows():
            lines.append(
                ...
                f"{int(r['Covers'])}-{int(r['ATS_Losses'])} ATS | {r['Cover_Pct']}% | "
                f"lift {r['Lift_vs_Family']} pts | "
                f"forward {int(r['Forward_Successes'])}/{int(r['Forward_Checks'])}"
            )

    (out_dir / "ATS_HEADLINE_RESULTS.txt").write_text("\n".join(lines), encoding="utf-8")

    print("", flush=True)
    print("COMPLETE", flush=True)
    print(f"Headline: {out_dir / 'HEADLINE_RESULTS.txt'}", flush=True)
    print(f"ATS actionable findings: {out_dir / 'actionable_findings.csv'}", flush=True)
    print(f"ATS pipeline implementation: {out_dir / 'pipeline_implementation.csv'}", flush=True)
    print(f"ATS historical feature matrix: {out_dir / 'historical_game_features.csv'}", flush=True)
