...
def _write_summary(summary: dict, per_file: list) -> None:
    lines = [
        ...
        "",
        f"  {'file':<48} {'market':<12} {'rows':>5} {'null_edges':>10} {'status':>14}",
    ]

    for pf in per_file:
        lines.append(
            f"  {pf['name']:<48} {pf['market']:<12} {pf['rows']:>5} "
            f"{pf['null_edges']:>10} {pf['status']:>14}"
        )
...
def assert_no_duplicate_columns(columns, label: str) -> None:
    dupes = duplicate_columns(list(columns))

    if dupes:
        raise RuntimeError(f"{label} has duplicate columns: {dupes}")


def assert_required_columns(df: pd.DataFrame, required_cols: list, label: str) -> None:
...
def validate_input_schema(df: pd.DataFrame, market: str, file_name: str) -> None:
    assert_no_duplicate_columns(df.columns, f"{file_name} input")

    if market == "moneyline":
        assert_required_columns(df, REQUIRED_MONEYLINE_COLS, f"{file_name} moneyline input")
    elif market == "run_line":
        assert_required_columns(df, REQUIRED_RUN_LINE_COLS, f"{file_name} run_line input")
    elif market == "total":
        assert_required_columns(df, REQUIRED_TOTAL_COLS, f"{file_name} total input")
    else:
        raise RuntimeError(f"{file_name} unknown market for schema validation: {market}")


def write_csv_guarded(df: pd.DataFrame, output_path: Path) -> None:
    assert_no_duplicate_columns(df.columns, f"{output_path} output")
...
def compute_total(df):
    df["over_prob"] = 1 / pd.to_numeric(
        df["fair_total_over_decimal"],
        errors="coerce",
    )

    df["under_prob"] = 1 / pd.to_numeric(
        df["fair_total_under_decimal"],
        errors="coerce",
    )
...
def main():
        ...
        try:
            ...
            if market in ("moneyline", "run_line"):
                if "sp_data_available" in df.columns and "lineup_data_available" in df.columns:
                    missing_sp = int((pd.to_numeric(df["sp_data_available"], errors="coerce") == 0).sum())
                    missing_lineup = int((pd.to_numeric(df["lineup_data_available"], errors="coerce") == 0).sum())

                    if missing_sp > 0:
                        _log(
                            f"{input_file.name} | {missing_sp} rows missing SP data — SP adjustment will use 0 diff",
                            "WARN",
                        )

                    if missing_lineup > 0:
                        _log(
                            f"{input_file.name} | {missing_lineup} rows missing lineup data — lineup/park adjustments will use 0 diff",
                            "WARN",
                        )
                else:
                    _log(
                        f"{input_file.name} | data availability columns not present — adjustments applied without indicators",
                        "WARN",
                    )
            ...
            write_csv_guarded(df, output_path)

            summary["files_processed"] += 1
            _log(f"WROTE: {output_path} ({len(df)} rows, {null_edges} null edges)")

        except RuntimeError as e:
            _log(f"{input_file.name} SCHEMA FAILED: {e}", "ERROR")
    ...
    if summary["errors"] > 0 or summary["schema_errors"] > 0:
        print("compute_edges completed with errors.")
        sys.exit(1)
