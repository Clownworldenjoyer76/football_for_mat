...
def read_parquet(path: Path) -> pd.DataFrame:
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not read parquet file {path}: {exc}"
        ) from exc
...
def choose_column(
    ...
    *,
    ...
    if required:
        raise ValueError(
            f"{label}: none of these columns are present: "
            f"{candidates}"
        )
...
def filter_unstarted_games(
    ...
    schedule_season = pd.to_numeric(
        schedule["season"],
        errors="coerce",
    )

    schedule_week = pd.to_numeric(
        schedule["week"],
        errors="coerce",
    )
    ...
    schedule_by_game = week_schedule.set_index(
        "game_id"
    )
    ...
    missing_schedule_ids = [
        ...
        if game_id not in schedule_by_game.index
    ]

    if missing_schedule_ids:
        raise ValueError(
            f"{schedule_path}: missing schedule rows for "
            f"season={season} week={week} game_id values: "
    ...
    if started_game_ids:
        base = base.loc[
            ~base["game_id"].isin(started_game_ids)
        ].copy()
        ...
        print(
            f"SKIPPED STARTED WEEK {week} GAMES: "
            + ", ".join(started_game_ids)
        )

    if base.empty:
        raise RuntimeError(
            f"{label}: no unstarted games remain "
...
def merge_game_source(
    ...
    source_ids = set(
        source["game_id"].dropna()
    )

    missing_ids = [
        gid
        for gid in base["game_id"]
    ...
    if missing_ids:
        raise ValueError(
            ...
            f"by combined input: {missing_ids[:10]}"
        )
...
def team_abbr(
    ...
    if (
        len(direct) <= 4
        and direct in set(lookup.values())
    ):
        return direct
    ...
    if key not in lookup:
        raise ValueError(
            f"Could not map NFL team to abbreviation: "
            f"{text!r}"
        )
    ...
    def __init__(
        ...
        for _, row in df.iterrows():
            ...
            record = PlayerRecord(
                ...
                position=(
                    position_for_grouping(
                        row[pos_col]
                    )
                    if pos_col
                    else position_for_grouping(
                        resolved.position
                        if resolved
                        else ""
                    )
...
class DepthSnapshot:
    ...
    def rank_for(
        ...
        if (
            gsis_id
            and gsis_id in self.rank_by_id
        ):
            return self.rank_by_id[gsis_id]

        return self.rank_by_name.get(
            normalize_name(name)
        )
    ...
    def position_for(
        ...
        if (
            gsis_id
            and gsis_id in self.position_by_id
        ):
            return self.position_by_id[
                gsis_id
            ]

        return self.position_by_name.get(
            normalize_name(name),
            "",
        )


def build_depth_snapshot(
    ...
    for record in records:
        identity = (
            record.gsis_id
            or (
                f"espn:{record.espn_id}"
                if record.espn_id
                else f"name:{normalize_name(record.name)}"
            )
        )
        ...
        if record.gsis_id:
            previous = rank_by_id.get(
                record.gsis_id
            )

            if (
                previous is None
                or record.rank < previous
            ):
                rank_by_id[
                    record.gsis_id
                ] = record.rank
...
def load_current_depth(
    ...
    for path in sorted(
        ...
        for _, row in df.iterrows():
            ...
            position = position_for_grouping(
                row["position_abb"]
            )
    ...
    if not output:
        raise RuntimeError(
            f"No current depth-chart rows "
            f"loaded from {depth_root}"
        )
...
class HistoricalDepthProvider:
    ...
    def __init__(
        ...
        self.available_weeks: dict[
            str,
            list[int],
        ] = {}

        self.timestamped: dict[
            str,
            list[
                tuple[
                    pd.Timestamp,
        ...
        if has_week:
            ...
            for (
                ...
                self.available_weeks.setdefault(
                    team,
                    [],
                ).append(week)
    ...
    def previous(
        ...
        if self.weekly:
            weeks = self.available_weeks.get(
                team,
                [],
            )
        ...
        for _, row in df.iterrows():
            ...
            offense_players = set(
                GSIS_PATTERN.findall(
                    clean(row[off_players_col])
                )
            )

            defense_players = set(
                GSIS_PATTERN.findall(
                    clean(row[def_players_col])
                )
            )
...
def load_current_injuries(
    ...
    path = (
        root
        / f"00_intake/injuries/{season}_injuries.csv"
    )
...
def starter_change_count(
    ...
    if (
        previous is None
        or not previous.starters_by_slot
        or not current.starters_by_slot
    ):
        return 0

    slots = (
        set(previous.starters_by_slot)
        | set(current.starters_by_slot)
    )
    ...
    return sum(
        ...
        for slot in slots
        if (
            previous.starters_by_slot.get(
                slot,
                frozenset(),
            )
            != current.starters_by_slot.get(
                slot,
                frozenset(),
            )
        )
...
def compute_injury_features(
    ...
    for injury in injuries:
        ...
        player = players.resolve(
            injury.raw_id,
            injury.name,
        )

        roster_player = (
            current_roster.resolve(
                injury.raw_id,
                injury.name,
            )
        )
...
def load_schema(
    ...
    if (
        len(feature_order)
        != len(set(feature_order))
    ):
        raise ValueError(
            f"{path}: feature_order contains "
            "duplicate names"
        )
    ...
    if (
        ...
        raise ValueError(
            f"{path}: numeric/categorical "
            "feature lists contain duplicates"
        )
    ...
    if (
        ...
        raise ValueError(
            f"{path}: numeric and categorical "
            "feature lists overlap"
        )
    ...
    if (
        ...
        raise ValueError(
            f"{path}: numeric/categorical "
            "feature sets do not exactly cover "
            "feature_order"
        )
...
def current_qb_starters(
    ...
    for _, row in df.iterrows():
        ...
        if (
            resolved is None
            or not resolved.gsis_id
        ):
            raise ValueError(
                f"{path}: could not map current "
                f"QB1 ESPN id {espn_id!r} for "
                f"{team} to GSIS id"
            )
...
def load_team_stats_for_week(
    ...
    path = (
        root
        / f"00_intake/team_stats/{source_season}_team_stats.csv"
    )
...
def load_qb_stats_for_week(
    ...
    path = (
        root
        / f"00_intake/qb/{source_season}_qb_stats.csv"
    )
...
def build_stadium_lookup(
    ...
    path = (
        root
        / "config/mapping/stadium_map_nfl.csv"
    )
...
def rest_days(
    ...
    if not candidates:
        raise ValueError(
            f"No prior current-season game "
            f"found for {team} before "
            f"Week {target_week}"
        )
...
def add_team_qb_features(
    ...
    for idx, row in work.iterrows():
        ...
        if (
            home not in team_stats
            or away not in team_stats
        ):
            raise ValueError(
                f"Missing required team-stat "
                f"source row for "
...
def add_schedule_features(
    ...
    for idx, row in work.iterrows():
        ...
        stadium_key = normalize_name(
            row["sched_stadium"]
        )

        team_key = normalize_name(
            row["base_home_team"]
        )
...
def prepare_week(
    ...
    if (
        ...
        raise ValueError(
            f"{combined_path}: expected only "
            f"season={season}, week={week}; "
            f"found seasons={season_values}, "
            f"weeks={week_values}"
        )
    ...
    if collisions:
        raise ValueError(
            f"{combined_path}: prediction columns "
            f"already exist and would be overwritten: "
            f"{collisions}"
        )
    ...
    if len(
        ...
        raise ValueError(
            f"{combined_path}: expected one "
            f"season_type, found {season_types}"
        )
    ...
    if week1_mode:
        ...
        prior_participation = (
            ParticipationProvider(
                read_parquet(
                    root
                    / "data/historic_data/participation"
                    / (
                        f"pbp_participation_"
                        f"{prior_season}.parquet"
                    )
                )
            )
        ...
        for required_path in [
            ...
            if not required_path.exists():
                raise FileNotFoundError(
                    f"Week {week} in-season "
                    f"projection requires "
                    f"current-season source: "
                    f"{required_path}"
                )

            if required_path.stat().st_size == 0:
                raise RuntimeError(
                    f"Required current-season "
                    f"source is zero bytes: "
                    f"{required_path}"
                )
    ...
    if missing_features:
        raise RuntimeError(
            f"Week {week}: could not construct "
            f"Step 11 features: "
            f"{missing_features}"
        )
    ...
    if (
        ...
        raise RuntimeError(
            f"Week {week}: expected "
            f"{EXPECTED_FEATURE_COUNT} "
            f"model features, found "
            f"{features.shape[1]}"
        )
    ...
    if (
        ...
        raise RuntimeError(
            f"Week {week}: feature names/order "
            "do not match Step 11 schema"
        )
    ...
    if not np.isfinite(
        ...
        raise ValueError(
            f"spread_line is "
            f"missing/non-numeric for "
            f"game_id values: {bad[:10]}"
        )
...
def load_calibrations(
    ...
    if (
        raw.get("method")
        != "one_variable_logistic_platt"
    ):
        raise ValueError(
            f"{path}: expected "
            f"method='one_variable_logistic_platt'; "
            f"found {raw.get('method')!r}"
        )
...
def apply_models(
    ...
    if (
        ...
        raise RuntimeError(
            "Prediction feature names/order "
            "do not match "
    ...
    if (
        len(predicted_margin)
        != len(original)
        or len(predicted_total)
        != len(original)
    ):
        raise RuntimeError(
    ...
    if not np.allclose(
        ...
        raise RuntimeError(
            "Predicted scores do not "
            "sum to predicted_total"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Final output columns are not "
            "original columns plus the exact "
            "10 prediction columns"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Final output game_id row order changed"
        )
...
def infer_inseason_target_week(
    ...
    If the newest week appearing in current-season team statistics contains
    only some of its scheduled teams, that week is still in progress and
    remains the projection target.

    Once every scheduled team for that week is represented in team statistics,
    the projection target advances to the following week.
    """
    path = (
        root
    ...
    if available.empty:
        raise RuntimeError(
            f"{path}: no current-season "
            f"team-stat weeks are available; "
            "use projection_week1.py for Week 1"
        )

    latest_stats_week = int(
        available.max()
    )
    ...
    if schedule_rows.empty:
        raise RuntimeError(
            f"{schedule_path}: no "
            f"regular-season schedule rows "
            f"for Week {latest_stats_week}"
        )
    ...
    if missing_teams:
        ...
        print(
            f"WEEK {latest_stats_week} "
            f"IS PARTIALLY COMPLETE; "
            f"KEEPING TARGET WEEK "
            f"{target_week} | "
            f"scheduled teams without "
            f"stats={missing_teams}"
        )
    ...
    combined_path = (
        root
        / "00_intake/predictions/enriched/combined"
        / f"week_{target_week}_NFL_enriched.csv"
    )

    if not combined_path.exists():
        raise FileNotFoundError(
            f"Projection target is "
            f"Week {target_week}, but its "
            f"combined input is missing: "
            f"{combined_path}"
        )
...
def run_projection(
    *,
    ...
    week = (
        1
        if week1_mode
        else infer_inseason_target_week(
            root,
            season,
        )
    )
