...
class EndToEndHistoricalAsCurrentTests(unittest.TestCase):
    ...
    def choose_historical_qb_state(self) -> pd.Series:
        columns = [
        ...
        candidates = frame.loc[
            ...
            & frame["week"].gt(1)
            & pd.to_numeric(frame["player_pass_attempts_lag1"], errors="coerce").notna()
            & pd.to_numeric(
    ...
    def test_historical_week_as_current_feature_projection_validation(self) -> None:
        chosen = self.choose_historical_qb_state()
        week = int(chosen["week"])
        player_id = common.normalize_player_id(chosen["player_id"])

        # Build current-style as-of features from the same historical season.
        # overlay_current_player_stats enforces source week < projection week.
        stats_rel = self.config["paths"]["historical_player_stats_pattern"].format(
            season=self.season
        )
        stats = pd.read_parquet(self.repo / stats_rel)
