...
def choose_qb(df_qb):
    # one QB per team: take the largest projection (likely starter)
    return df_qb.groupby(["season","week","team","opponent"], as_index=False)["qb_passing_yards"].max()

def sum_rb(df_rb):
    # sum all RB rushing yards per team
    return df_rb.groupby(["season","week","team","opponent"], as_index=False)["rb_rushing_yards"].sum()

def canon_pair(a, b):
    # simple deterministic home/away by name
